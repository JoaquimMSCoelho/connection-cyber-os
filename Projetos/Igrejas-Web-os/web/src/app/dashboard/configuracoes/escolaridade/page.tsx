...
import { ArrowLeft, Plus, Loader2, Trash2, Library } from "lucide-react";
import Link from "next/link";
import { addSettingAction, deleteSettingAction } from "../actions";

export default function EscolaridadeConfigPage() {
  ...
  const TABLE_NAME = "settings_schooling";
  ...
  async function handleDelete(id: string) {
    if (!confirm("Tem certeza que deseja excluir esta escolaridade?")) return;
    ...
    <div className="max-w-[800px] mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col gap-4 pb-6 border-b border-neutral-800">
        ...
        <div className="flex items-center gap-3">
          <div className="p-3 rounded-lg bg-neutral-900 border border-neutral-800 text-indigo-500"><Library className="w-6 h-6" /></div>
          <div><h1 className="text-2xl font-bold text-white tracking-tight">Escolaridade</h1><p className="text-sm text-neutral-400">Níveis de base de conhecimento.</p></div>
        </div>
      </div>
        ...
        <form action={handleAdd} className="flex gap-3 items-start">
            <div className="flex-1">
                <input 
                    ...
                    className="w-full bg-neutral-950 border border-neutral-800 rounded-lg p-3 text-white focus:border-indigo-500 outline-none"
                    required
                />
                {error && <span className="text-red-500 text-xs font-bold mt-2 block">{error}</span>}
            </div>
            <button type="submit" disabled={isSubmitting || !newName.trim()} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white px-6 py-3 rounded-lg text-sm font-medium transition-colors">
        ...
        {loading ? (
            ...
            <table className="w-full text-left text-sm text-neutral-400">
                <thead className="bg-neutral-950 text-xs uppercase text-neutral-500 border-b border-neutral-800">
                    <tr>
                        <th className="px-6 py-4 font-medium">Nível de Escolaridade</th>
