...
import { ArrowLeft, Loader2, Trash2, Briefcase, AlertCircle } from "lucide-react";
import Link from "next/link";
import { addSettingAction, deleteSettingAction } from "../actions";

export default function CargosConfigPage() {
  ...
  const TABLE_NAME = "ecclesiastical_roles";
  ...
  async function handleDelete(id: string) {
    if (!confirm("Atenção: Tem certeza que deseja excluir este cargo do sistema?")) return;
    ...
    <div className="max-w-[1200px] mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-xl flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black text-white tracking-tight uppercase">CARGOS</h1>
          </div>
        </div>

        <form action={handleAdd} className="flex flex-col sm:flex-row gap-3 items-center w-full lg:w-auto">
            <input 
                type="text" name="name" placeholder="Ex: PASTOR PRESIDENTE" value={newName} onChange={(e) => setNewName(e.target.value)} 
                className="w-full sm:w-80 bg-neutral-950 border border-neutral-800 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/50 transition-all font-medium uppercase placeholder:normal-case placeholder:text-neutral-700" required
            />
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-xl overflow-hidden shadow-sm flex flex-col">
        {loading ? (
            <div className="flex flex-col justify-center items-center py-16 gap-4"><Loader2 className="w-10 h-10 text-emerald-500 animate-spin" /><p className="text-sm font-bold uppercase tracking-wider text-neutral-500">Sincronizando Banco...</p></div>
        ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                  <thead className="bg-neutral-950 text-[10px] uppercase font-bold text-neutral-500 border-b border-neutral-800">
                      <tr><th className="px-6 py-4 tracking-wider">Descrição do Registro</th><th className="px-6 py-4 text-right tracking-wider">Ação</th></tr>
                  </thead>
                          ...
                          items.map((item) => (
                              <tr key={item.id} className="hover:bg-neutral-800/80 transition-colors group">
                                  ...
                                  <td className="px-6 py-4 text-right w-20">
                                      <button onClick={() => handleDelete(item.id)} className="p-2 text-neutral-500 hover:text-red-500 hover:bg-neutral-800 rounded-lg transition-colors inline-flex items-center justify-center"><Trash2 className="w-4 h-4" /></button>
                                  </td>
                              </tr>
                          ))
                      )}
