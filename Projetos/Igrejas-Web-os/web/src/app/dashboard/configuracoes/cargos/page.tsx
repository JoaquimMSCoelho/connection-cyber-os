...
import { ArrowLeft, Plus, Loader2, Trash2, ShieldCheck } from "lucide-react";
    ...
    <div className="max-w-[800px] mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col gap-4 pb-6 border-b border-neutral-800">
        ...
        <div className="flex items-center gap-3">
          <div className="p-3 rounded-lg bg-neutral-900 border border-neutral-800 text-purple-500"><ShieldCheck className="w-6 h-6" /></div>
          <div><h1 className="text-2xl font-bold text-white tracking-tight">Cargos</h1><p className="text-sm text-neutral-400">Níveis de autoridade eclesiástica.</p></div>
        </div>
      </div>
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-5">
        <form action={handleAdd} className="flex gap-3 items-start">
            ...
            <button type="submit" disabled={isSubmitting || !newName.trim()} className="flex items-center gap-2 bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white px-6 py-3 rounded-lg text-sm font-medium transition-colors">
        ...
        {loading ? (
            ...
            <table className="w-full text-left text-sm text-neutral-400">
                <thead className="bg-neutral-950 text-xs uppercase text-neutral-500 border-b border-neutral-800">
                    <tr>
                        <th className="px-6 py-4 font-medium">Nome do Cargo</th>
                        <th className="px-6 py-4 font-medium text-right">Ação</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-neutral-800">
                    {items.length === 0 ? (
                        <tr><td colSpan={2} className="px-6 py-8 text-center">Nenhum cargo cadastrado.</td></tr>
                    ) : (
                        items.map((item) => (
